...
* EAP types (Identity, TLS, TTLS, PEAP, MSCHAPv2…)
* Timestamps
* EAP Start
* Outer TLS observed
...
* `anon.pcap` — MACs rewritten
* `mapping.json` — original → pseudonym mapping

Pseudonyms are derived using:

```
HMAC_SHA256(seed, mac) → 6-byte address
```
...
* `type=None` → EAP Start
* `type=1` → Identity
* `type={13,21,25}` → TLS/TTLS/PEAP
* `EAPOL.type==3` → 4-way handshake presence
